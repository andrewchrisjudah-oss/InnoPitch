...
function StudyAssistant({ user }: { user: User }) {
  ...
  return <div className="pointer-events-none fixed inset-x-0 bottom-5 z-50">
    ...
    <button onClick={() => setOpen(!open)} aria-label={open ? "Close Baymax study assistant" : "Open Baymax study assistant"} className={`${open ? "pointer-events-auto absolute right-5 bottom-0" : "baymax-runner pointer-events-auto absolute bottom-0 left-0"} relative grid size-14 overflow-hidden rounded-full text-white shadow-xl transition hover:scale-105 ${open ? "bg-red-700 shadow-red-900/40" : "bg-[#6B1F3A] shadow-[#6B1F3A]/30"}`}>
      {open ? <MessageCircle className="relative z-10 size-6" /> : <video src="/baymax-intro.mp4#t=0,8" autoPlay loop muted playsInline className="size-full object-cover" />}
    </button>
  </div>;
}
