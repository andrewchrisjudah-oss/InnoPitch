...
function StudyAssistant({ user }: { user: User }) {
  ...
  return <div className="pointer-events-none fixed inset-x-0 bottom-3 z-50">
    {open && <div className="pointer-events-auto mb-3 ml-auto mr-5 flex h-[28rem] w-[min(22rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-3xl border border-[#C98F9F]/30 bg-[#0B0B0D]/95 text-[#F7F1EE] shadow-2xl backdrop-blur-xl">
    ...
    <button onAnimationEnd={() => { if (runRequested && !runnerDone) { setRunnerDone(true); setOpen(true); setRunRequested(false); wakeTrigger.current = false; greet(); } }} onClick={() => { if (open) closeChat(); else setOpen(true); }} aria-label={open ? "Close Baymax study assistant" : "Open Baymax study assistant"} className={`${open ? "pointer-events-auto absolute right-5 bottom-0 rounded-full bg-red-700 p-3" : `${runRequested ? "baymax-runner" : "baymax-idle"} pointer-events-auto absolute bottom-0 left-0 ${runnerDone ? "baymax-stopped" : ""}`} relative overflow-hidden`}>
