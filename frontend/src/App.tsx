...
function StudyAssistant({ user }: { user: User }) {
  ...
  return <div className="fixed bottom-5 right-5 z-50">
    ...
    <button onClick={() => setOpen(!open)} aria-label={open ? "Close Baymax study assistant" : "Open Baymax study assistant"} className={`relative grid size-14 overflow-hidden rounded-full text-white shadow-xl transition hover:scale-105 ${open ? "bg-red-700 shadow-red-900/40" : "bg-[#6B1F3A] shadow-[#6B1F3A]/30"}`}>
