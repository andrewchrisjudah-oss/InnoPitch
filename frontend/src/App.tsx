...
function StudyAssistant({ user }: { user: User }) {
  ...
  return <div className="pointer-events-none fixed inset-x-0 bottom-3 z-50">
    ...
    <button onAnimationEnd={() => { if (runRequested && !runnerDone) { setRunnerDone(true); setOpen(true); setRunRequested(false); wakeTrigger.current = false; greet(); } }} onClick={() => { if (open) closeChat(); else if (!wakeEnabled) toggleWakeWord(); }} aria-label={open ? "Close Baymax study assistant" : "Baymax is listening"} className={`${open ? "pointer-events-auto absolute right-5 bottom-0 rounded-full bg-red-700 p-3 shadow-red-900/40" : `${runRequested ? "baymax-runner" : "baymax-idle"} pointer-events-auto absolute bottom-0 left-0 ${runnerDone ? "baymax-stopped" : ""}`} relative overflow-hidden transition hover:scale-105`}>
